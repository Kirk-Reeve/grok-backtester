...
from numpy import inf, nan, sqrt
...
def calculate_metrics(portfolio: DataFrame) -> Dict[str, float]:
    ...
    Args:
        portfolio (DataFrame): A pandas DataFrame with a DatetimeIndex,
    ...
    try:
        ...
        logger.debug("Calculating metrics for %s returns", len(returns))
        ...
        cagr = (
            (total.iloc[-1] / total.iloc[0]) ** (252 / num_days) - 1
        ...
        std_dev = returns.std()
        sharpe_ratio = (
            (mean_return * 252) / (std_dev * sqrt(252)) if std_dev != 0 else 0.0
        )

        downside_returns = returns[returns < 0]
        downside_std = downside_returns.std() if not downside_returns.empty else 0.0
        sortino_ratio = (
            (mean_return * 252) / (downside_std * sqrt(252))
            if downside_std != 0
            else 0.0
        )
        ...
        logger.error("Unexpected error in metrics: %s", error)
        raise MetricsError(f"Metrics calculation failed: {error}") from error
